...
function displayLinkedinUserData(user) {
    const myTable = objectToTable(user);

    document.getElementById("linkedin-user-data").innerHTML = myTable;
}
...
function objectToTable(obj) {
    ...
    for (let label in labelToProperty) {
        ...
        // Check if the property is an object with a 'localized' property
        if (typeof obj[property] === 'object' && obj[property] !== null && obj[property].hasOwnProperty('localized')) {
            let preferredLocale = obj[property].preferredLocale;
            let localeKey = preferredLocale.language + "_" + preferredLocale.country;
            let localizedValue = obj[property].localized[localeKey];
            table += "<tr><td style='font-weight:bold'>" + label + "</td><td>" + localizedValue + "</td></tr>";
        }
...
function google_dorks(user){
    ...
    const full_name = user.localizedFirstName + " " + user.localizedLastName;
